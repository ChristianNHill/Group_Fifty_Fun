...
<form action="forumaddpost.php" method="get">
  <button style="padding:10px"; width="20%">Create new post<strong>+</strong></button>
</form>
</br></br>
</div>
...
<div>
	...
	while($row  = getArray($result)) {
		...
		if(isset($_SESSION['logged_in'])){
			...
			$user_id=$_SESSION['id'];
			$result = query("SELECT value from vote WHERE user_id=$user_id AND link_id=$post_id;");
			if($result){
				$vote_value = getArray($result);
				if(!empty($vote_value)){
					if($vote_value[0] == 1){
